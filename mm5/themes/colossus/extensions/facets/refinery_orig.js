...
(function ($, window) {
	...
	$.fn.checkFacetOverflow = function () {
		if ($facets) {
			...
			if (requiredSpace > availableSpace && window.innerWidth > minDisplayWidth) {
			...
			else if (availableSpace > breakWidths[numOfVisibleItems] && window.innerWidth > minDisplayWidth) {
				// There is more than enough space
				$hiddenLinks.children().first().appendTo($visibleLinks).insertBefore($triggerArea);
				numOfVisibleItems += 1;
				$facets.addClass('is-loaded');
				this.checkFacetOverflow();
			}
